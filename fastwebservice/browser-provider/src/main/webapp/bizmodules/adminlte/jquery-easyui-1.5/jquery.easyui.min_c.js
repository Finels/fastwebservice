        ...
        function _224() {
            if (opts.noheader || (!opts.title && !opts.header)) {
                ...
                if (opts.header) {
                    ...
                    var _228 = _223.children(".panel-header");
                    if (!_228.length) {
                        _228 = $("<div class=\"panel-header top_header clearfix\" style=\"padding:5px;\"></div>").prependTo(_223);
                    }
                    if (!$.isArray(opts.tools)) {
                        _228.find("div.panel-tool .panel-tool-a").appendTo(opts.tools);
                    }
                    _228.empty();
                    //创建div以便于bootstrap自适应
                    var lefttitle
                    if (opts.tools) {
                        lefttitle = $("<div class='left col-sm-4 col-xs-12 clearfix'></div>");
                    }else {
                        lefttitle = $("<div class='left col-sm-12 col-xs-12 clearfix'></div>");
                    }

                    lefttitle.appendTo(_228);
                    if(opts.iconUrl){
                        $("<em><img src='"+opts.iconUrl+"'/></em>").appendTo(lefttitle)
                    }
                    var _229 = $("<span class=\"panel-title\"></span>").html(opts.title).appendTo(lefttitle);
                    if (opts.iconCls) {
                        _229.addClass("panel-with-icon");
                        $("<div class=\"panel-icon\"></div>").addClass(opts.iconCls).appendTo(_228);
                    }
                    var tool = $("<div class=\"panel-tools  right col-xs-12 col-sm-7\"></div>").appendTo(_228);
                    tool.bind("click", function (e) {
                        e.stopPropagation();
                    });
                    if (opts.tools) {
                        if ($.isArray(opts.tools)) {
                            //添加ul标签在工具上
                            tool = $("<ul class='table_title clearfix'></ul>").appendTo(tool)
                            $.map(opts.tools, function (t,index) {
                                function bindtools(c, t, handler){
                                    var a;
                                    //判断是否为最后一个按钮
                                    if(index == (opts.tools.length-1)){
                                        a = $("<li class=\"last clearfix\"><button class='tools-btn' href=\"javascript:void(0)\"></button></li>").appendTo(c); //将图标放到工具栏上
                                    }else{
                                        a = $("<li class=\"clearfix\"><button class='tools-btn' href=\"javascript:void(0)\"></button></li>").appendTo(c); //将图标放到工具栏上
                                    }
                                    a = a.find("button");
                                    if(t.id)
                                    {
                                    ...
                                    //给按钮添加图标
                                    if(t.iconUrl){
                                        $("<em><img src='"+ t.iconUrl+"'></em>").appendTo(a)
                                    }
                                    $("<span></span>").text(t.text).appendTo(a)
                                    a.bind("click", handler); //注册点击事件
                                    //a.linkbutton($.extend({}, t, {plain: true})); //绑定linkbutton配置
                                }
                                bindtools(tool, t, eval(t.handler));
                            });
                        } else {
                            $(opts.tools).children().each(function () {
                                $(this).addClass($(this).attr("iconCls")).addClass("panel-tool-a").appendTo(tool);
...
(function ($) {
    ...
    var _7b7 = {
        render: function (_7b8, _7b9, _7ba) {
            ...
            var vc = $(_7f7).datagrid('getPanel').children('div.datagrid-view');
            vc.children('div.datagrid-empty').remove();
            if (!$(_7f7).datagrid('getRows').length){
