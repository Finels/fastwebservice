...
(function ($) {
    window.ScdpMessage = function () {
        var success_html = '<div id="[Id]" class="modal fade" role="dialog" aria-labelledby="modalLabel">' +
            ...
            '<div class="modal-content popup-msg-top-boarder popup-msg-bottom-boarder">' +
            '<div class="bg-green color-palette popup-msg-top-boarder" style="height: 40px;"> <h4 style="color: white;padding:10px;"><i class="icon fa fa-check"></i> [Title]</h4> </div>' +
            '<div class="modal-body">' +
            '<p>[Message]</p>' +
            '</div>' +
            '<div class="modal-footer popup-msg-bottom-boarder">' +
        ...
        var warning_html = '<div id="[Id]" class="modal fade" role="dialog" aria-labelledby="modalLabel">' +
            '<div class="modal-dialog modal-sm" style="margin:200px auto">' +
            '<div class="modal-content popup-msg-top-boarder popup-msg-bottom-boarder">' +
            '<div class="bg-yellow color-palette popup-msg-top-boarder" style="height: 40px;"> <h4 style="color: white;padding:10px;"><i class="icon fa fa-warning"></i> [Title]</h4> </div>' +
            '<div class="modal-body">' +
            '<p>[Message]</p>' +
            '</div>' +
            '<div class="modal-footer popup-msg-bottom-boarder">' +
        ...
        var error_html = '<div id="[Id]" class="modal fade" role="dialog" aria-labelledby="modalLabel">' +
            '<div class="modal-dialog modal-sm" style="margin:200px auto">' +
            '<div class="modal-content popup-msg-top-boarder popup-msg-bottom-boarder">' +
            '<div class="bg-red color-palette popup-msg-top-boarder" style="height: 40px;"> <h4 style="color: white;padding:10px;"><i class="icon fa fa-ban"></i> [Title]</h4> </div>' +
            '<div class="modal-body">' +
            '<p>[Message]</p>' +
            '</div>' +
            '<div class="modal-footer popup-msg-bottom-boarder">' +
...
$(document).ready(function () {
    $("form.scdpActionForm").submit(function (e) {
        var postData = {};
        ...
        e.preventDefault();
        $("body").append('<form class="hidden" id="'+ formId + '" action="scdp/controller/jsp/load-tasks" method="post">' +
            "</form>");
        ...
        $("#userId", $(formatFormId)).val(Scdp.CacheUtil.get(Scdp.Const.USER_ID));
        $("#userLocaleId", $(formatFormId)).val(Scdp.getSysConfig("locale_id"));
        $("#timestamp", $(formatFormId)).val(Date.now());
        $("#network", $(formatFormId)).val(window.NETWORK_DELAY);
        $("#actionName", $(formatFormId)).val(action);
        $("#signature", $(formatFormId)).val(Scdp.getSign(action, postData));
 ...
 * @param fn
 */
Scdp.Msg.toastr_info = function (message, fn) {
    toastrInit(3000);;
 ...
 * @param fn
 */
Scdp.Msg.toastr_warn = function (message, fn) {
    toastrInit(3600);
 ...
 * @param fn
 */
Scdp.Msg.toastr_error = function (message, fn) {
    toastrInit(3500);
