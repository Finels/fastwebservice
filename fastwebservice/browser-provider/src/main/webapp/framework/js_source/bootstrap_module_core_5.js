 ...
 */
Namespace.register("Scdp.MainFrameEvents");
Namespace.register("Scdp.Loader");
Namespace.register("Scdp.ModuleManager");

Scdp.MainFrameEvents = {
    eventObj: {},
    addListener: function(eventName, fn) {
        var fns;
        if(Scdp.ObjUtil.isEmpty(this.eventObj)) {
    ...
    fireEvent: function(eventName) {
        ...
        var ret = undefined;
        if(Scdp.ObjUtil.isNotEmpty(this.eventObj)) {
            if(this.eventObj.hasOwnProperty(eventName)) {
                var fns = this.eventObj[eventName];
                if(Scdp.ObjUtil.isNotEmpty(fns)) {
 ...
 * @returns
 */
Scdp.Loader.setPath = function(name, path) {
    var modules = Scdp.Loader.modules ||{};
    if(name) {
        modules[name] = path;
        Scdp.Loader.modules = modules;
    }
};
 ...
 * @returns path 加载路径
 */
Scdp.Loader.getPath = function(name) {
    var modules = Scdp.Loader.modules;
    if(name && modules) {
        return modules[name];
    }
...
Scdp.ModuleManager.getObject = function(key) {
    var objects = Scdp.ModuleManager.objects;
    if(objects && key) {
        return objects[key];
    } else {
        return null;
...
Scdp.ModuleManager.putObject = function(key, value) {
    var objects = Scdp.ModuleManager.objects ||{};
    if(key) {
        objects[key] = value;
        Scdp.ModuleManager.objects = objects;
    }
};
Scdp.copy = function(dest, org, deep) {
    if(deep) {
...
Scdp.extend = function (parent, child) {
    var tempObj = Object.create(parent);
    child = Scdp.copy(tempObj, child);
    child._super = parent;
    return child;
};

Scdp.define = function(objName, obj){
    if(Scdp.ModuleManager.getObject(objName)) {
        return;
    }
    if(obj instanceof Object && obj.extend) {
        var parentStr = obj.extend;
        Scdp.loadComponent(parentStr)
            .done(function(parentObj){
                obj._compName = objName;
                obj = Scdp.extend(parentObj, obj);
                if(obj.mixins && $.isArray(obj.mixins)) {
                    var count = 0;
                    $.each(obj.mixins, function(i, v) {
                        Scdp.loadComponent(v).done(function(mixObj){
                            var tempObj = Object.create(mixObj);
                            obj = Scdp.copy(tempObj, obj);
                            count = i;
                        });
                    });
                    if(obj.mixins.length == (count + 1)) {
                        Scdp.ModuleManager.putObject(objName, obj);
                        Scdp.Loader.notifyFun(objName,[obj]);
                        return;
                    }
                }
                Scdp.ModuleManager.putObject(objName, obj);
        ...
        obj.callParent = function() {
            ...
            if (Scdp.ObjUtil.isEmpty(methodName)) {
...
Scdp.loadComponent = function(module) {
    var dtd = $.Deferred();
    if(module) {
        var existModule = Scdp.ModuleManager.objects[module];
        if(existModule) {
            dtd.resolve(Scdp.copy({}, existModule, true));
            return dtd.promise();
        } else {
            var prefix = module.slice(0, module.indexOf("."));
            var prefixName = Scdp.Loader.modules[prefix];
            ...
            var getTimestamp= Scdp.DateUtil.formatDate(dateNow, Scdp.Const.BUSINESS_DATE_FORMAT);
            modulePath = window.SYSCONFIG_BASE_PATH  + splitChar + modulePath.replace(/\./g, "/") + ".js" + "?v="+getTimestamp + ".js";
            try {
                Scdp.Loader.load(modulePath, function(){
                    var moduleObj = Scdp.ModuleManager.getObject(module);
                    if(moduleObj) {
                        dtd.resolve(Scdp.copy({}, moduleObj, true));
                    } else {
                        if(!moduleObj) {
                            var times = 0;
                            var t = setInterval(function() {
                                ...
                                moduleObj = Scdp.ModuleManager.getObject(module);
                                if(moduleObj) {
                                    clearInterval(t);
                                    dtd.resolve(Scdp.copy({}, moduleObj, true));
                                }
...
$(document).ready(function() {
    Scdp.bootstrapParse = function(context, controller, menuCode, postParse){
        var dtd = $.Deferred();
        var postParseList = [];
        if(Scdp.comps_bs) {
            $.each(Scdp.comps_bs, function(i, type){
                var comps = $("[xtype="+ type + "]", context);
                if(comps.length > 0) {
                    comps.each(function () {
                        try {
                            var compObj = Scdp.ModuleManager.getObject(type);
                            if(compObj) {
                                compObj = Scdp.copy({},compObj, true);
                            } else {
                                if($.inArray(type, Scdp.value_comps_bs) != -1) {
                                    compObj = Scdp.copy({}, Scdp.ModuleManager.getObject("scdp_value_comp"), true);
                                } else {
                                    compObj = Scdp.copy({}, Scdp.ModuleManager.getObject("scdp_comp"), true);
                                }
                            }
                            ...
                            var itemId = Scdp.Utils.getCompKey($(this));
                            ...
                            var itemId = Scdp.Utils.getCompKey($(this));
                            Scdp.DebugUtil.logErr(" parse component " + itemId +" : " + error.stack);
                        }
                    });
                }
            });
            ...
            setTimeout(function(){
                $.each(postParseList, function(i, comp){
                    ...
                    try {
                        ...
                        Scdp.DebugUtil.logErr(" parse component " + name +" : " + error.stack);
                    }
                });
                dtd.resolve();
            },10);
    ...
    Scdp.parseComponent = function(ele, options, context, controller, menuCode){
        try {
            var type = $(ele).attr("xtype");
            if(Scdp.ObjUtil.isEmpty(type)) {
                type = options.xtype;
            }
            if(Scdp.ObjUtil.isEmpty(type)) {
                return;
            ...
            compObj.ele = ele;
            compObj.context = context;
            var itemId = Scdp.Utils.getCompKey($(ele));
            if(Scdp.ObjUtil.isEmpty(itemId)) {
                itemId = options.itemId;
            }
            var initBefore = new Date().getTime();
            $(ele).data(type, compObj);
            if($(ele)[type]) {
                $(ele)[type]("_init", options);
            }
            ...
            obj[type] = function(method) {
                if(typeof method == "string") {
                    try {
                        ...
                        var name = Scdp.Utils.getCompKey($(this));
                        Scdp.DebugUtil.logErr("component " + name +" call method " + method + ": " + error.stack);
                    }
                }
            };
    ...
    $.fn.extend({
        getCmp: function(itemId, contex) {
            ...
            if (Scdp.ObjUtil.isNotEmpty(itemId)) {
                var itemIds = Scdp.StrUtil.split(itemId,"->");
                var itemIdSelector = "";
                var idSelector = "";
                var nameSelector = "";
                ...
                $.each(compEles, function(j, compEle){
                    ...
                    if(defineObj) {
                        try {
                            ...
                            Scdp.DebugUtil.logErr("component " + defineObj.itemId +" destroy fail : " + error.stack);
                        }
                    }
                });
            }
    ...
    if($.fn.dataTable && $.fn.dataTable.defaults) {
        $.extend( true, $.fn.dataTable.defaults, {
            language: {
                url: 'framework/js/3rdparty/plugins/dataTables/lang/'+ Scdp.getSysConfig("locale_id")+ '.json'
            },
            paging:false,
            searching:false,
            autoWidth:false,
    ...
    if($.fn.datagrid) {
        function pagerFilter(data){
            ...
            if (opts.clientPage && opts.sortName){
                var names = Scdp.StrUtil.split(opts.sortName,',');
                var orders = Scdp.StrUtil.split(opts.sortOrder,',');
        ...
        $.fn.datagrid.methods = $.extend($.fn.datagrid.methods, {
            ...
            loadData: function(jq, data){
                if(Scdp.ObjUtil.isNotEmpty(data) && !$.isArray(data) && data.rows == null) {
                    return;
                }
        ...
        var comboxEditor = Scdp.ModuleManager.getObject("edit_comb");
        $.fn.datagrid.defaults.editors.combcol = $.extend($.extend({},$.fn.datagrid.defaults.editors.combobox),comboxEditor);

        var combtreeEditor = Scdp.ModuleManager.getObject("edit_combtree");
        $.fn.datagrid.defaults.editors.combtree = $.extend($.extend({},$.fn.datagrid.defaults.editors.combotree),combtreeEditor);

        var dateboxEditor = Scdp.ModuleManager.getObject("edit_datebox");
        $.fn.datagrid.defaults.editors.datebox = $.extend($.extend({},$.fn.datagrid.defaults.editors.datebox),dateboxEditor);

        var datetimeEditor = Scdp.ModuleManager.getObject("edit_datetimebox");
        $.fn.datagrid.defaults.editors.datetimebox = $.extend($.extend({},$.fn.datagrid.defaults.editors.datetimebox),datetimeEditor);

        var searchboxEditor = Scdp.ModuleManager.getObject("edit_searchbox");
    ...
    if($.fn.textbox) {
        $.fn.textbox.parseOptions=function(_65){
            ...
            if(options.required && Scdp.ObjUtil.isNotEmpty(options.label) && typeof options.label =='string') {
                options.label = "<span class='star-after' title='"+options.label +"'>"+options.label+" </span>";
            } else if(Scdp.ObjUtil.isNotEmpty(options.label) && typeof options.label =='string') {
        ...
        $.fn.tree.defaults.view.render = function (domEle, ul, data) {
                ...
                function renderNode(spans,dataItems) {
                    ...
                    for (var i = 0; i < dataItems.length; i++) {
                        ...
                        item.domId = "_easyui_tree_" + Scdp.StrUtil.getUUID();
                        cc.push("<li>");
                        if(item.disabled) {
    ...
    if($.fn.textbox) {
        $.fn.textbox.defaults.cls = "scdp-textbox";
    }
    if($.fn.combo) {
        $.fn.combo.defaults.cls = "scdp-textbox";
    }
    if($.fn.combobox) {
        $.fn.combobox.defaults.cls = "scdp-textbox";
    }
    if($.fn.passwordbox) {
        $.fn.passwordbox.defaults.cls = "scdp-textbox";
    }
    if($.fn.combotree) {
        $.fn.combotree.defaults.cls = "scdp-textbox";
    }
    if($.fn.combogrid) {
        $.fn.combogrid.defaults.cls = "scdp-textbox";
    }
    if($.fn.combotreegrid) {
        $.fn.combotreegrid.defaults.cls = "scdp-textbox";
    }
    if($.fn.tagbox) {
        $.fn.tagbox.defaults.cls = "scdp-textbox";
    }
    if($.fn.numberbox) {
        $.fn.numberbox.defaults.cls = "scdp-textbox";
    }
    if($.fn.datebox) {
        $.fn.datebox.defaults.cls = "scdp-textbox";
    }
    if($.fn.datetimebox) {
        $.fn.datetimebox.defaults.cls = "scdp-textbox";
    }
    if($.fn.datetimespinner) {
        $.fn.datetimespinner.defaults.cls = "scdp-textbox";
    }
    if($.fn.spinner) {
        $.fn.spinner.defaults.cls = "scdp-textbox";
    }
    if($.fn.numberspinner) {
        $.fn.numberspinner.defaults.cls = "scdp-textbox";
    }
    if($.fn.timespinner) {
        $.fn.timespinner.defaults.cls = "scdp-textbox";
    }
    if($.fn.filebox) {
        $.fn.filebox.defaults.cls = "scdp-textbox";
    }
    if($.fn.searchbox) {
        $.fn.searchbox.defaults.cls = "scdp-textbox";
    }
        ...
        $.extend($.fn.validatebox.defaults.rules, {
            ...
            comboTreeIsExist: {
                validator: function(value,param){
                    ...
                    if(Scdp.ObjUtil.isEmpty(records)) {
                        return false;
    ...
    if($.fn.bootstrapTable && $.fn.bootstrapTable.Constructor) {
    ...
    $("body").on("mouseover", 'div.datagrid-cell', function(e) {
        ...
        if(Scdp.ObjUtil.isNotEmpty(tValue)) {
