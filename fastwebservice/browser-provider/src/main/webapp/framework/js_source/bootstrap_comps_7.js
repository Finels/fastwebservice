 ...
 */
Scdp.value_comps_ext = [];
Scdp.no_value_comps_bs_ext = [];

Scdp.value_comps_bs = ['bPanel', 'bForm', 'bText', 'bHidden', 'bLabel', 'bInteger','bDecimal','bDate','bDateTime', 'bCheckBox', 'bRadioBox','bTel','bFax','bPhone',
    ...
    'e_datetimespinner','e_timespinner','e_filebox','e_searchbox','e_combobox','e_calendar','e_datalist','e_tree','e_tab','e_datagrid','e_editgrid','bt_table'];
Scdp.no_value_comps_bs = ['bBtn', 'e_linkbutton','checkbox_label','e_tablist'];
Scdp.value_comps =  Scdp.value_comps_bs.concat(Scdp.value_comps_ext);
Scdp.no_value_comps = Scdp.no_value_comps_bs.concat(Scdp.no_value_comps_bs_ext);
Scdp.comps_bs = Scdp.value_comps_bs.concat(Scdp.no_value_comps_bs);
Scdp.comps = Scdp.value_comps_bs.concat(Scdp.no_value_comps_bs, Scdp.value_comps_ext, Scdp.no_value_comps_bs_ext);
Scdp.Utils.getCompKey = function (ele) {
    var key = null;
...
Scdp.Utils.parseOptions = function (s) {
    var options = {};
    if (s){
        if (s.substring(0, 1) != '{'){
            s = '{' + s + '}';
...
Scdp.define("scdpNavTab", {
    select: function(tabId) {
        ...
        if(selTab.length > 0) {
            ...
            Scdp.CacheUtil.setTemp(Scdp.Const.CACHE_ACTIVE_MODULE, Scdp.Const.CACHE_ACTIVE_MODULE_KEY, tabId);
            Scdp.updateRoute(tabId);
        ...
        if(newTab instanceof Object) {
            ...
            if(newTab.hasOwnProperty("tabId") && newTab.hasOwnProperty("title") && newTab.hasOwnProperty("content")) {
                ...
                var tabTemplate = '<li id="'+ newTab.tabId + '" style="position: relative;"><a href="#content_'+ newTab.tabId + '" data-toggle="tab"><em></em>' +
                    '<span class="bs-tab-title">'+ newTab.title;
                if(!isindex){
                    tabTemplate = tabTemplate + '</span>&nbsp;&nbsp;<span class="fa fa-times-circle"></span>';
                }
                tabTemplate = tabTemplate + '</a><em></em><i class="arrow-right"></i></li>';
    ...
    closeTab: function(ele) {
        ...
        Scdp.CacheUtil.setTemp(Scdp.Const.CACHE_MODULE_TAB, tabId, null);
...
Scdp.define("scdp_comp", {
    ...
    _init: function(options) {
        var name = Scdp.Utils.getCompKey(this.ele);
        if(Scdp.ObjUtil.isEmpty(name) && options && $.isPlainObject(options)) {
            name = options.itemId;
        }
        this.itemId = name;
        this.UNIQUE_KEY = Scdp.StrUtil.getUUID();
        if(name && Scdp.ObjUtil.isEmpty(this.ele.attr("name"))) {
        ...
        if(options && $.isPlainObject(options)) {
            ...
            if(optionstr !== "" && optionstr !== null && optionstr !== undefined) {
                try {
                    this.options = Scdp.Utils.parseOptions(optionstr);
                } catch(e) {
                    if(!name) {
                        name = $(this.ele).attr("field");
                    }
                    Scdp.DebugUtil.logInfo("component " + this.xtype + " " + name + " parse options error!");
                }
        ...
        this.options = $.extend(true,{}, Scdp.DefaultOptions[this.xtype] || {}, this.options);
        if(this.commonInit) {
            this.commonInit();
        }
        this.init();
...
Scdp.define("scdp_value_comp", {
    extend:'scdp_comp',
    commonInit: function() {
        var filterFields = this.ele.attr("filterFields");
        var cascadeField = this.ele.attr("cascadeField");
        var target = this.ele.attr("target");

        if(Scdp.ObjUtil.isNotEmpty(filterFields)) {
            this.filterFields = filterFields;
            this.options.filterFields = this.filterFields;
        } else {
            this.filterFields = Scdp.StrUtil.replaceNull(this.options.filterFields);
        }
        if(Scdp.ObjUtil.isNotEmpty(cascadeField)) {
            this.cascadeField = cascadeField;
            this.options.cascadeField = this.cascadeField;
        } else {
            this.cascadeField = Scdp.StrUtil.replaceNull(this.options.cascadeField);
        }
        if(Scdp.ObjUtil.isNotEmpty(target)) {
            this.target = target;
            this.options.target = this.target;
        } else {
            this.target = Scdp.StrUtil.replaceNull(this.options.target);
        }
    },
    gotValue: function() {
        return this.ele.val();
    ...
    cascadeLoad: function (forceFul, isFilterEmpty) {
        var me = this;
        if (Scdp.ObjUtil.isNotEmpty(me.cascadeField)) {
            var upForm = me.ele.parents("[xtype='bForm'],[xtype='bPanel']");
            var cascadeFieldArr = Scdp.StrUtil.split(me.cascadeField,",");
            for (var i = 0; i < cascadeFieldArr.length; i++) {
                var cascadeObj = null;
                cascadeObj = upForm.getCmp(cascadeFieldArr[i]);
                if (Scdp.ObjUtil.isNotEmpty(cascadeObj) && cascadeObj.length > 0) {
    ...
    refreshTarget: function (obj) {
        if (Scdp.ObjUtil.isNotEmpty(obj.target)) {
            var targetFields = Scdp.StrUtil.split(obj.target,",");
            $.each(targetFields, function (i, item) {
                ...
                if (Scdp.ObjUtil.isNotEmpty(targetObj) && targetObj.length >0) {
                    if (Scdp.ObjUtil.isNotEmpty(record)) {
                        var rData = record;
                        if (Scdp.ObjUtil.isNotEmpty(itemMapping[1])) {
                            if($.isArray(rData)) {
                                var separator = Scdp.StrUtil.replaceNull(obj.options.separator, "|");
                                var values = [];
...
Scdp.define("bPanel", {
    ...
    gotValue: function() {
        ...
        $.each(Scdp.value_comps, function(index, type){
            ...
            if(subComps.length > 0) {
                $.each(subComps, function(index, subComp){
                    ...
                    var key = Scdp.Utils.getCompKey($(subComp));
                    if('e_tab' == xtype  || ('bPanel' == xtype || 'bForm' == xtype) && (Scdp.ObjUtil.isEmpty(key) || ($(subComp).data(xtype).options && $(subComp).data(xtype).options.skipValue))) {
    ...
    sotValue: function(obj) {
        ...
        if(this.xtype ==="bForm" && Scdp.ObjUtil.isEmpty(obj)) {
                ...
                $.each(subComps, function(index, subComp){
                    ...
                    if('bForm' == type || 'bPanel' == type) {
                        ...
                        if($.inArray(ret.xtype,["e_combo","e_combobox","e_combotree","e_combogrid"]) != -1) {
                            ...
                            if(options.multiple) {
                                ret.separator = Scdp.StrUtil.replaceNull(options.separator, "|");
                            }
                        }
...
Scdp.define("bForm", {
    ...
    init: function() {
        ...
        var action = this.ele.attr("action");
        if(Scdp.ObjUtil.isEmpty(action)) {
            this.ele.submit(function () {
                return false;
            });
        }
        if($.fn.form) {
    ...
    gotInvalidBoxLabel: function () {
        if($.fn.form) {
            var invalidbox = this.ele.find('.validatebox-invalid');
            var firstBox = invalidbox.filter(':not(:disabled):first');
            var orgInput = firstBox.parents(".textbox").prev();
            if(orgInput.attr("label")) {
                return orgInput.attr("label");
            } else {
                return "";
            }
        } else {
...
Scdp.define("bRadioGroup", {
    ...
    init: function(){
        ...
        if(Scdp.ObjUtil.isEmpty(codeType)) {
            codeType = Scdp.StrUtil.replaceNull(this.options.codeType);
        }
        var values = this.ele.attr("values");
        if(Scdp.ObjUtil.isEmpty(values)) {
            values = Scdp.StrUtil.replaceNull(this.options.values);
        }
        var key = Scdp.Utils.getCompKey(this.ele);
        ...
        var data = null;
        if(Scdp.ObjUtil.isNotEmpty(codeType)) {
            data = Scdp.getComboStoreDate("scdp_fmcode", codeType, me.menuCode, null, me.options.needCache);
        } else if(Scdp.ObjUtil.isNotEmpty(values)) {
            data = eval('('+values +')');
        }
        ...
        if(this.options.defaultValue != null || this.options.defaultValue != undefined) {
            this.sotValue(this.options.defaultValue);
        }
    },
    destroy: function() {
    ...
    sotDisableItem: function(values) {
        if(Scdp.ObjUtil.isNotEmpty(values) && $.isArray(values)) {
...
Scdp.define("bCheckGroup", {
    ...
    sotValue: function(value) {
        var valueLst = [];
        if(Scdp.ObjUtil.isNotEmpty(value) && typeof value === 'string') {
            valueLst = Scdp.StrUtil.split(value,",");
        }
        $("[name='"+this.groupName+"']").each(function() {
            var v = $(this).val();
...
Scdp.define("bComboxSin", {
    ...
    init: function () {
        ...
        if(Scdp.ObjUtil.isEmpty(placeholder)) {
            placeholder = Scdp.StrUtil.replaceNull(this.options.placeholder);
        }
        var me = this;
        if(!this.combType) {
        ...
        if(Scdp.ObjUtil.isEmpty(liveSearch)) {
            liveSearch = Scdp.StrUtil.replaceNull(this.options.liveSearch);
        }
        if("false" ===liveSearch) {
            this.options.minimumResultsForSearch = Infinity;
        }
        if(Scdp.ObjUtil.isNotEmpty(placeholder)) {
        ...
        if(Scdp.ObjUtil.isEmpty(tags)) {
            tags = Scdp.StrUtil.replaceNull(this.options.tags);
        }
        if("true" ===tags) {
    ...
    reload: function (upContainer, record) {
        ...
        if(listData && listData.length>0) {
            ...
            var tempList = $.map(listData, function (obj) {
                ...
                if(Scdp.ObjUtil.isEmpty(obj.id) || Scdp.ObjUtil.isEmpty(obj.text)) {
                } else {
                    return obj;
                }
            });
...
Scdp.define("bTable", {
    ...
    init: function () {
        ...
        cols.each(function(i, col) {
            ...
            if(colOptionStr) {
                var colOptions = Scdp.Utils.parseOptions(colOptionStr);
                ...
                field.render = function(data, type, row, meta) {
                    if(data) {
                        if("createTime" === field.data || "updateTime" === field.data) {
                            data = Scdp.DateUtil.parseDateToLocal(data);
                        }
            ...
            if("bTcol" === xtype || "bTcolExt" === xtype) {
                if(Scdp.ObjUtil.isNotEmpty(format)) {
                    if(!!me[format]) {
            ...
            if("bTnumCol" === xtype) {
                ...
                field.render = function(data, type, row, meta) {
                    if (data) {
                        return Scdp.Utils.formatNumber(data, precision);
                    } else {
                        return data;
                    }
                }
        ...
        if(Scdp.ObjUtil.isEmpty(pagingStr)) {
            pagingStr = Scdp.StrUtil.replaceNull(this.options.paging);
        }
        if("true" === pagingStr) {
        ...
        if(Scdp.ObjUtil.isEmpty(searchingStr)) {
            searchingStr = Scdp.StrUtil.replaceNull(this.options.searching);
        }
        ...
        if(Scdp.ObjUtil.isEmpty(scrollY)) {
        ...
        if(Scdp.ObjUtil.isEmpty(selectStr)) {
            selectStr = Scdp.StrUtil.replaceNull(this.options.select);
        }
        ...
        if(Scdp.ObjUtil.isEmpty(needCheck)) {
            needCheck = Scdp.StrUtil.replaceNull(this.options.needCheck);
        }
        if("true" === needCheck) {
            needSelect = true;
            if(Scdp.ObjUtil.isEmpty(selectStyle)) {
                selectStyle = "multi";
            }
        ...
        if(Scdp.ObjUtil.isEmpty(needRowNum)) {
            needRowNum = Scdp.StrUtil.replaceNull(this.options.needRowNum);
        }
        if("true" === needRowNum) {
        ...
        if(Scdp.ObjUtil.isEmpty(needFooter)) {
            needFooter = Scdp.StrUtil.replaceNull(this.options.needFooter);
        }
        if("true" === needFooter) {
        ...
        this.options.footerCallback = function( tfoot, data, start, end, display) {
            ...
            if(me.options.needFooter && $(tfoot).has("td").length ==0) {
                $.each(tempColumns, function(i, col) {
                    var itemId = "footer_" + Scdp.StrUtil.replaceNull(col.data,"");
                    if(col.bVisible) {
    ...
    _addDefaultOrder: function() {
        ...
        if(Scdp.ObjUtil.isEmpty(orders)) {
            orders = me.options.orders;
        }
        var order = [];
        if(Scdp.ObjUtil.isNotEmpty(orders)) {
            try {
                var ordersObj = Scdp.Utils.parseOptions(orders);
                if(Scdp.ObjUtil.isNotEmpty(ordersObj) && typeof ordersObj === 'object') {
                ...
                Scdp.DebugUtil.logErr(orders + " table order format is wrong! please order to {userId:'asc',userName:'desc'}");
            }
        }
        if(order.length > 0) {
    ...
    loadDataPage: function(param, queryAction) {
        ...
        var newOptions = {serverSide: true, ajax:function (data, callback, settings) {
            ...
            if(Scdp.ObjUtil.isEmpty(tempParam)) {
                return;
            }
            data = $.extend(data, tempParam);
            data.aggfuns = me.aggfuns;
            Scdp.loadFreeMarkerAction(queryAction, data, function(retData) {
                callback(retData);
                if(retData.footRow) {
                    me.sotFooterValue(retData.footRow);
                }
            }, function(e) {
                Scdp.DebugUtil.logErr("get table data error!");
            } );
        }};
        ...
        function tempSetValue () {
            ...
            $.each(footerRow, function(key, value) {
                ...
                if(td.length > 0 ) {
                    ...
                    if("bTnumCol" ==xtype) {
                        value = Scdp.Utils.formatNumber(value, precision);
                    }
                }
                td.empty().html(value);
            });
...
Scdp.define("bDate", {
    ...
    init: function () {
        ...
        if(Scdp.ObjUtil.isEmpty(max)) {
            max = me.options.max_d;
        }
        if(max) {
            var tempDateMom = moment(max);
            if(Scdp.ObjUtil.isNotEmpty(tempDateMom)) {
                options.endDate = tempDateMom.toDate();
            }
        }
        var min = this.ele.attr("min_d");
        if(Scdp.ObjUtil.isEmpty(min)) {
            min = me.options.min_d;
        }
        if(min) {
    ...
    sotValue: function(value) {
        if(value !=null && value instanceof Date) {
            this.ele.val(Scdp.DateUtil.formatDate(value));
        } else if(Scdp.ObjUtil.isNotEmpty(value) && typeof value === 'string') {
            var tempDateMom = moment(value);
            if(Scdp.ObjUtil.isNotEmpty(tempDateMom)) {
                this.ele.val(tempDateMom.format("YYYY-MM-DD"));
            }
        } else if(Scdp.ObjUtil.isEmpty(value)) {
            this.ele.val(null);
        }
    },
...
Scdp.define("bInteger", {
    extend: 'scdp_value_comp',
    init: function () {
        var me = this;
        var showzero = me.ele.attr("showzero");
        if(Scdp.ObjUtil.isEmpty(showzero)) {
            showzero = Scdp.StrUtil.replaceNull(this.options.showzero);
        }
        var options = {};
        if("false" != showzero) {
    ...
    gotValue: function() {
        ...
        if(value) {
            value = Scdp.StrUtil.replaceAll(value, ",", "");
        }
        return value;
    }
});
Scdp.define("bDecimal", {
    extend: 'scdp_value_comp',
    init: function () {
        var me = this;
        var showzero = me.ele.attr("showzero");
        ...
        if(Scdp.ObjUtil.isEmpty(precision)) {
        ...
        if(precision) {
            ...
            if(precision.indexOf(",") != -1) {
                var precis = Scdp.StrUtil.split(precision, ",");
...
Scdp.define("checkbox_label", {//新加checkbox左label禁用启用方法
    extend: 'scdp_comp',
    init: function () {
...
Scdp.define("bAhead", {
    ...
    init: function () {
        ...
        if(Scdp.ObjUtil.isEmpty(valuefrom)) {
            valuefrom = this.options.valuefrom;
        }
        if(Scdp.ObjUtil.isNotEmpty(valuefrom)) {
    ...
    gotRecord:function() {
        ...
        if(Scdp.ObjUtil.isEmpty(record)) {
            return null;
        } else {
    ...
    gotValue: function() {
        //if(Scdp.ObjUtil.isNotEmpty(this.valuefrom) && this.valuefrom !== 'codedesc') {
...
Scdp.define("bFile", {
    ...
    init: function () {
        ...
        var options = {
            language: Scdp.getSysConfig("LOCALE_ID"),
            uploadUrl:Scdp.Const.JSON_ACTION
        };
        var fileExt = me.ele.attr("fileExt");
        if(Scdp.ObjUtil.isEmpty(fileExt)) {
        ...
        if(fileExt) {
            ...
            if(commaIndex) {
                options.allowedFileExtensions = Scdp.StrUtil.split(fileExt,",");
            } else {
                options.allowedFileExtensions = Scdp.StrUtil.split(fileExt,"|");
            }
        }
        var maxFileCount = me.ele.attr("maxFileCount");
        if(Scdp.ObjUtil.isEmpty(maxFileCount)) {
        ...
        if(Scdp.ObjUtil.isEmpty(maxFileSize)) {
    ...
    _upload: function(e) {
        ...
        if(Scdp.ObjUtil.isEmpty(fileName)) {
            if(me.fnExtUpdate) {
        ...
        if(Scdp.ObjUtil.isEmpty(me.extraParams.fileName)) {
            me.extraParams.fileName = fileName;
        }
        if(Scdp.ObjUtil.isEmpty(me.extraParams.fileType)) {
        ...
        Scdp.doAction(me.extraParams.actionName, me.extraParams, function(data) {
            if(me.fnSuccess) {
                me.fnSuccess(data);
            }
    ...
    _registEvents: function() {
        ...
        me.ele.off('filepreajax').on('filepreajax', function(event, previewId, index) {
            ...
            if(Scdp.ObjUtil.isEmpty(param.fileType)) {
                param.fileType = fileType;
            }
            param.fileItemId = me.itemId;
            param.userId = Scdp.CacheUtil.get(Scdp.Const.USER_ID);
            param.userLocaleId = Scdp.getSysConfig("locale_id");
            param.timestamp = Date.now();
            param.network = window.NETWORK_DELAY;
            if(!param.actionName) {
                param.actionName = "file-upload";
            }
            param.signature = Scdp.getSign(param.actionName, param);
            var postData = Scdp.JSON.encode(param);
...
Scdp.define("bTree", {
    ...
    init: function () {
        ...
        if(Scdp.ObjUtil.isEmpty(this.loadAction)) {
            this.loadAction = Scdp.StrUtil.replaceNull(this.options.loadAction);
        }

        var showRoot = this.ele.attr("showRoot");
        if(Scdp.ObjUtil.isEmpty(showRoot)) {
        ...
        if(Scdp.ObjUtil.isEmpty(onlyLeaf)) {
            onlyLeaf = this.options.onlyLeaf;
        }
        if(onlyLeaf && "true" == onlyLeaf) {
            this.onlyLeaf = true;
        }
        var parentIcon = this.ele.attr("parentIcon");
        if(Scdp.ObjUtil.isEmpty(parentIcon)) {
            parentIcon = this.options.parentIcon;
        }
        if(parentIcon) {
            this.parentIcon = parentIcon;
        }
        var leafIcon = this.ele.attr("leafIcon");
        if(Scdp.ObjUtil.isEmpty(leafIcon)) {
        ...
        if(Scdp.ObjUtil.isEmpty(multiple)) {
        ...
        if(actionParams) {
            this.actionParams = Scdp.Utils.parseOptions(actionParams);
        }
        var chkboxType = this.ele.attr("chkboxType");
        if(Scdp.ObjUtil.isEmpty(chkboxType)) {
            chkboxType = this.options.chkboxType;
        }
        if(chkboxType) {
            this.chkboxType = Scdp.Utils.parseOptions(chkboxType);
        }
        var bindid = this.ele.attr("bindid");
        if(Scdp.ObjUtil.isEmpty(bindid)) {
            bindid = this.options.bindid;
        }
        if(bindid) {
            this.bindid = bindid;
        }
        var binddesc = this.ele.attr("binddesc");
        if(Scdp.ObjUtil.isEmpty(binddesc)) {
            binddesc = this.options.binddesc;
        }
        if(binddesc) {
    ...
    _getTreeData: function() {
        ...
        var retData = Scdp.doAction(me.loadAction, params, function(retData){
            var test = retData;
        },null,null,false);
        if(me.showRoot) {
            return retData;
    ...
    appendTextValue: function(value) {
        if(Scdp.ObjUtil.isNotEmpty(value)) {
            ...
            if(Scdp.ObjUtil.isNotEmpty(existText)) {
    ...
    sotValue: function(value) {
        ...
        if(Scdp.ObjUtil.isEmpty(value)) {
            ...
            var values = Scdp.StrUtil.split(value,",");
            me.treeObj.checkAllNodes(false);
